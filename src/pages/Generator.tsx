...
const EXAMPLE_PROMPTS: PromptTemplate[] = [
  ...
  {
    category: "Travel",
    label: "Souvenir scanner",
    tagline: "Turn trip objects into a 3D memory shelf.",
    signature: "Object Capture → AR shelf you can revisit.",
    screens: ["Shelf", "Scan", "Trip detail", "AR room"],
    accent: "#F08A5D",
    emoji: "🗿",
    prompt:
      "A travel keepsake app: scan small souvenirs with Object Capture (PhotogrammetrySession) to build 3D models, organized into trip 'shelves' you can walk through in AR via RealityKit. Includes a map of where each item was collected and a yearly recap reel.",
  },
  {
    category: "Food",
    label: "Fridge alchemist",
    tagline: "What can I cook right now?",
    signature: "Vision-tagged fridge photo → on-device recipe ideas.",
    screens: ["Fridge", "Snap fridge", "Recipe", "Shopping list"],
    accent: "#E84545",
    emoji: "🥘",
    prompt:
      "A 'what can I cook?' app. Snap your fridge — Vision identifies ingredients, on-device AI proposes 3 recipes ranked by what you already have. SwiftData tracks pantry, expiry-soon Live Activity reminds you, App Intents add missing items to a shopping list via Siri.",
  },
  {
    category: "Music",
    label: "Field recorder atlas",
    tagline: "A world map of your ambient recordings.",
    signature: "Geo-pinned waveforms + soundscape playlists.",
    screens: ["Atlas", "Record", "Recording detail", "Playlist"],
    accent: "#3D5A80",
    emoji: "🎙",
    prompt:
      "A field-recording journal: capture ambient sound with AVAudioEngine, auto-tag with location, weather, and time of day. MapKit atlas pins each recording with a mini waveform; build soundscape playlists and a Lock Screen widget that plays today's most-recent capture.",
  },
  {
    category: "Reading",
    label: "Margin notes",
    tagline: "A social book club of marginalia.",
    signature: "Anonymous highlights drift past as you read.",
    screens: ["Library", "Reader", "Margins", "Discussions"],
    accent: "#8D6E63",
    emoji: "📖",
    prompt:
      "A reading companion where you log books and capture margin notes (text + voice). Anonymous highlights from other readers drift past in the margins as you read the same passage. CloudKit-synced library, beautiful serif typography, optional book-club rooms.",
  },
  {
    category: "Fitness",
    label: "Trail tarot",
    tagline: "Each run draws you a movement card.",
    signature: "Generated tarot card from your route + pace.",
    screens: ["Deck", "Today's card", "Run", "History"],
    accent: "#9B5DE5",
    emoji: "🃏",
    prompt:
      "A running app that ends each session by generating a unique 'tarot' card representing the run — illustration based on route shape, pace and elevation. HealthKit + CoreMotion power the data; SwiftUI Canvas renders cards; build a deck over the year and share favorites.",
  },
  {
    category: "Home",
    label: "Tiny home inventory",
    tagline: "Every object you own, beautifully cataloged.",
    signature: "Room-by-room AR scan + one-tap insurance export.",
    screens: ["Rooms", "Object detail", "AR scan", "Export"],
    accent: "#2EC4B6",
    emoji: "🏠",
    prompt:
      "A home inventory app: scan rooms with RoomPlan, tag objects via camera + Vision, store value/serial/photo in SwiftData. One-tap PDF export for insurance, warranty expiry Live Activities, and a 'what's in the attic?' search across rooms.",
  },
  {
    category: "Mindfulness",
    label: "Worry parking lot",
    tagline: "Park anxious thoughts; revisit when ready.",
    signature: "Cars drive into a lot — open the trunk later.",
    screens: ["Lot", "Park a worry", "Trunk", "Weekly review"],
    accent: "#577590",
    emoji: "🅿️",
    prompt:
      "A CBT-inspired anxiety tool: 'park' worries as little cars in a SwiftUI parking lot. Each car holds a thought, severity, and a future check-in date. On the date, a Live Activity invites you to open the trunk and reappraise. Weekly insights show patterns.",
  },
  {
    category: "Kids",
    label: "Bedtime story forge",
    tagline: "Co-create a nightly story with your kid.",
    signature: "Pick 3 cards → on-device AI weaves the tale.",
    screens: ["Tonight", "Card picker", "Story", "Library"],
    accent: "#FF9F1C",
    emoji: "🦊",
    prompt:
      "A bedtime story co-creator for parents and kids. Pick 3 illustrated cards (hero / place / twist) and on-device Foundation Models weave a 4-minute story read aloud with AVSpeechSynthesizer. Save favorites to a SwiftData library, with gentle dimming and night-mode UI.",
  },
  {
    category: "Work",
    label: "Meeting weather",
    tagline: "A forecast for your calendar week.",
    signature: "EventKit → storms, sunshine, deep-work windows.",
    screens: ["Forecast", "Day detail", "Focus windows", "Widget"],
    accent: "#118AB2",
    emoji: "🌦",
    prompt:
      "Reads your EventKit calendar and renders the week as a weather forecast: meeting density = clouds, back-to-back blocks = storms, free chunks = sunshine. Suggests deep-work windows, blocks them with one tap, and offers a Lock Screen widget showing today's outlook.",
  },
  {
    category: "Curiosity",
    label: "Object oracle",
    tagline: "Point your camera. Hear its story.",
    signature: "Vision → on-device AI tells the object's micro-history.",
    screens: ["Viewfinder", "Story", "Collection", "Map"],
    accent: "#06D6A0",
    emoji: "🔮",
    prompt:
      "Point your camera at any everyday object and the app spins a 60-second narrated micro-history (origin, materials, cultural notes) using Vision + on-device AI + AVSpeechSynthesizer. Save oracles to a collection, pin where you found them on a map, share as audio cards.",
  },
];
